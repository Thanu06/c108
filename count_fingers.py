...
def countFingers(image, hand_landmarks, handNo=0):
    ...
    if hand_landmarks:
        ...
        for im_index in tipIds:
            fingerTipY=landmarks[im_index].y
            fingerBottomY=landmarks[im_index].y
            if im_index!=4:
                if fingerTipY<fingerBottomY:
                    fingers.append(1)
                    print("finger width id",im_index,"is open")
                if fingerTipY>fingerBottomY:
                    fingers.append(0)
                    print("finger width id",im_index,"is closed")   
        totFinger=fingers.count(1)
        text=f"fingers:{totFinger}"
